(function () {
  ...
  CreateCampaignController.$inject = ['$scope', '$state', '$window', 'Authentication', 'WidgetsService', '$firebaseStorage', '$firebaseAuth', 'UsersService', 'Upload', 'FirebaseService', 'Notification', '$base64', 'CampaignsService', '$stateParams'];

  function CreateCampaignController($scope, $state, $window, Authentication, WidgetsService, $firebaseStorage, $firebaseAuth, UsersService, Upload, FirebaseService, Notification, $base64, CampaignsService, $stateParams) {
    var _this = this;
    ...
    _this.createCampaign = function () {
      var campaign = {
        ...
        widget: _this.widget._id
      };

      CampaignsService.createCampaign(campaign).then(function (response) {
        $state.transitionTo('campaigns.list', $stateParams, { reload: true, inherit: false, notify: true });
